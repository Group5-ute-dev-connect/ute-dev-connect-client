...
const Groups = () => {
  ...
  const fetchGroups = async () => {
    try {
      setLoading(true);
      const response = await groupApi.getAllGroups(1, 100, searchQuery);
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-grow">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {/* Search bar & Tabs */}
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="relative flex-grow max-w-md">
              ...
              <input
                ...
                className="block w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-all duration-200"
              />
            </div>
          </div>
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="mt-3 text-gray-500 text-sm font-medium">Đang tải danh sách nhóm...</p>
            </div>
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {filteredGroups.map((group) => {
                ...
                return (
                  <div 
                    key={group._id} 
                    className="bg-white rounded-2xl border border-gray-100 hover:border-indigo-100 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between overflow-hidden group/card"
                  >
                    <div className="p-6">
                      {/* Name & Admin badge */}
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <h3 className="text-lg font-bold text-gray-900 group-hover/card:text-indigo-600 transition-colors line-clamp-1">
                          {group.name}
                        </h3>
                        {ownGroup && (
                      ...
                      <div className="flex items-center justify-between text-xs text-gray-500 pt-4 border-t border-gray-50">
                        <div className="flex items-center space-x-1">
                          ...
                          <span>Admin: <strong className="font-medium text-gray-700">{adminName}</strong></span>
                        </div>
                        <div className="flex items-center space-x-1">
                    ...
                    {/* Actions */}
                    <div className="px-6 py-4 bg-gray-50 border-t border-gray-50 flex items-center justify-between gap-2">
                      <div>
                        {ownGroup && (
                          <button
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl border border-gray-100 overflow-hidden transform scale-100 transition-all">
            ...
            <form onSubmit={handleCreateGroup} className="p-6">
              ...
              <div className="mb-4">
                <label htmlFor="group-name" className="block text-xs font-bold text-gray-700 mb-1.5 uppercase tracking-wider">
                ...
                <input
                  ...
                  className="block w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-colors"
                  maxLength={100}
                />
              </div>

              <div className="mb-6">
                <label htmlFor="group-desc" className="block text-xs font-bold text-gray-700 mb-1.5 uppercase tracking-wider">
                  Mô tả nhóm
                </label>
                <textarea
                  id="group-desc"
                  ...
                  rows={4}
                  className="block w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-colors resize-none"
                  maxLength={500}
                />
              </div>
